...
export async function hasLocalAccount(db: SQLiteDatabase) {
  const row = await db.getFirstAsync<{ count: number }>('SELECT COUNT(*) AS count FROM accounts');
  return (row?.count ?? 0) > 0;
}

export async function createLocalAccount(db: SQLiteDatabase, input: CreateAccountInput) {
...
export async function getActiveAccountId(db: SQLiteDatabase) {
