@echo off
setlocal
cd /d "%~dp0"

echo ========================================
echo   AUSleepApp Test Build
echo ========================================
echo.

where node >nul 2>&1
if errorlevel 1 (
  echo Node.js was not found.
  echo Install the current Node.js LTS version, then run this file again.
  pause
  exit /b 1
)

where npm >nul 2>&1
if errorlevel 1 (
  echo npm was not found.
  echo Reinstall Node.js LTS with npm enabled, then run this file again.
  pause
  exit /b 1
)

if not exist node_modules (
  echo Installing project packages. This can take several minutes on the first run...
  call npm install
  if errorlevel 1 (
    echo.
    echo Package installation failed. Review the messages above.
    pause
    exit /b 1
  )
)

echo.
echo Starting Expo...
echo Scan the QR code for Expo Go, or press W after Expo starts to open the web version.
echo.
call npx expo start -c

pause
